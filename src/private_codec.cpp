...
Result<ContractOrder> contract_order(const WireContractOrder &wire,
                                     std::string prefix) {
  if (!wire.hash) return missing(prefix + ".hash");
  if (!wire.salt) return missing(prefix + ".salt");
  if (!wire.maker) return missing(prefix + ".maker");
  if (!wire.signer) return missing(prefix + ".signer");
  if (!wire.taker) return missing(prefix + ".taker");
  if (!wire.tokenId) return missing(prefix + ".tokenId");
  if (!wire.makerAmount) return missing(prefix + ".makerAmount");
  if (!wire.takerAmount) return missing(prefix + ".takerAmount");
  if (!wire.expiration) return missing(prefix + ".expiration");
  if (!wire.nonce) return missing(prefix + ".nonce");
  if (!wire.feeRateBps) return missing(prefix + ".feeRateBps");
  if (!wire.side) return missing(prefix + ".side");
  if (!wire.signatureType) return missing(prefix + ".signatureType");
  if (!wire.signature) return missing(prefix + ".signature");
  ...
  if (!maker) { auto e = maker.error(); e.field = prefix + ".maker"; return e; }
  if (!signer) { auto e = signer.error(); e.field = prefix + ".signer"; return e; }
  if (!token) return token.error();
  if (!maker_amount) return maker_amount.error();
  if (!taker_amount) return taker_amount.error();
  if (!expiration) return expiration.error();
  ...
  if (*wire.taker != "0x0000000000000000000000000000000000000000") {
    ...
    if (!parsed) { auto e = parsed.error(); e.field = prefix + ".taker"; return e; }
    taker = parsed.value();
  }
  ...
  if (*wire.signatureType == 0U) signature_type = SignatureType::eoa;
  else if (*wire.signatureType == 1U) signature_type = SignatureType::poly_proxy;
  else if (*wire.signatureType == 2U)
    signature_type = SignatureType::poly_gnosis_safe;

  return ContractOrder{*wire.hash, std::move(salt.value()), maker.value(),
                       signer.value(), taker, std::move(token.value()),
                       std::move(maker_amount.value()),
                       std::move(taker_amount.value()),
                       std::move(expiration.value()), std::move(nonce.value()),
                       std::move(fee.value()),
                       enum_value(side, std::to_string(*wire.side)),
                       enum_value(signature_type,
                                  std::to_string(*wire.signatureType)),
                       *wire.signature};
}
...
Result<Account> decode_account_response(std::string_view json,
  ...
  if (!data.name) return missing("data.name");
  if (!data.address) return missing("data.address");
  ...
  if (!address) { auto e = address.error(); e.field = "data.address"; return e; }
  ...
  if (data.referral) {
    ...
    account.referral = ReferralInfo{*data.referral->code,
                                    *data.referral->status};
  }
  if (data.points) {
    if (!data.points->total) return missing("data.points.total");
...
Result<PositionsPage> decode_positions_response(std::string_view json,
  ...
  for (std::size_t index = 0; index < wire.value().data->size(); ++index) {
    ...
    if (!item.outcome) return missing(prefix + ".outcome");
    if (!item.amount) return missing(prefix + ".amount");
    if (!item.valueUsd) return missing(prefix + ".valueUsd");
    if (!item.averageBuyPriceUsd)
      return missing(prefix + ".averageBuyPriceUsd");
    if (!item.pnlUsd) return missing(prefix + ".pnlUsd");
    ...
    auto average = exact(*item.averageBuyPriceUsd,
                         prefix + ".averageBuyPriceUsd");
    auto pnl = exact(*item.pnlUsd, prefix + ".pnlUsd");
    if (!parsed_market) return parsed_market.error();
    if (!parsed_outcome) return parsed_outcome.error();
    if (!amount) return amount.error();
    if (!value) return value.error();
    if (!average) return average.error();
    if (!pnl) return pnl.error();
    page.positions.push_back(Position{
        *item.id, std::move(parsed_market.value()),
...
Result<ActivityPage> decode_activity_response(std::string_view json,
  ...
  for (std::size_t index = 0; index < wire.value().data->size(); ++index) {
    const auto &item = (*wire.value().data)[index];
    const auto prefix = "data[" + std::to_string(index) + "]";
    if (!item.name) return missing(prefix + ".name");
    if (!item.createdAt) return missing(prefix + ".createdAt");
    ActivityEvent event{*item.name, *item.createdAt, item.transactionHash,
                        {}, {}, {}, {}, {}};
    ...
    if (item.order) {
      ...
      ActivityOrder order{*item.order->quoteType, std::move(amount.value()),
                          std::move(price.value()), {}};
      if (item.order->fee) {
        if (!item.order->fee->amount || !item.order->fee->type)
          return missing(prefix + ".order.fee");
        auto fee = exact(*item.order->fee->amount,
                         prefix + ".order.fee.amount");
        if (!fee) return fee.error();
        order.fee = ActivityFee{std::move(fee.value()),
                               *item.order->fee->type};
      }
